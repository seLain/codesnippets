        ...
        for i in range(0, n-1):
            ...
            if current_temp_idx == len(ordered_temperatures)-1: # no more higher temperature
                ...
                for higher_temp in ordered_temperatures[current_temp_idx+1:]:
                    # using binary searcg to find nearst index
                    p = bisect.bisect_left(dict_temperatures[higher_temp], i)
                    if p == len(dict_temperatures[higher_temp]):
                        continue
                    else:
                        if dict_temperatures[higher_temp][p] < nearest_higher_idx:
                            nearest_higher_idx = dict_temperatures[higher_temp][p]
