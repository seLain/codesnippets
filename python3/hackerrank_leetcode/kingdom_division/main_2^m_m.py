...
def kingdomDivision(n, roads):
    ...
    # consider only the nodes in roads
    mapping = defaultdict(int)
    new_index = 1
    for r in roads:
        if r[0] not in mapping.keys():
            mapping[r[0]] = new_index
            new_index += 1
        r[0] = mapping[r[0]]
        if r[1] not in mapping.keys():
            mapping[r[1]] = new_index
            new_index += 1
        r[1] = mapping[r[1]] 
    n = len(mapping.keys())
    
    # start kingdom division
    peace_counter = 0
    for i in range(0, pow(2,n)):
        ...
        for r in roads:
            graph.add_edge(r)
        ## check if all nodes are peace, if yes, add up the counter
        if graph.all_peace():
            peace_counter += 1
